...
def resolve_under_snap(snap: Path, raw: str) -> Path:
  ...
  if not p.is_absolute():
    ...
    if p_r.exists():
      ...
      # Best-effort remap:
      # 1) If raw contains the snapshot dir name, strip up to it.
      parts = list(p.parts)
      cand = p_r
...
def main():
  ...
  # DB hash: always validate the actual db inside snap-dir, regardless of manifest db.path text.
