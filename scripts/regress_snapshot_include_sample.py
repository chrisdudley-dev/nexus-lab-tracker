...
def newest_snapshot_dir(exports_dir: Path) -> Path:
    snaps = sorted(
        [d for d in exports_dir.iterdir() if d.is_dir() and d.name.startswith("snapshot-")],
        key=lambda d: d.name
    )
    if not snaps:
        raise RuntimeError("FAIL: no snapshot-* directory created")
    return snaps[-1]
...
def main() -> int:
    ...
    # One sample (keeps this regression focused and avoids container exclusivity issues)
    ...
    snap = newest_snapshot_dir(exports_dir)
